...
// The J -- pion wall loop in disconnected diagram
LatticeLoop loops_contraction(const std::vector<LatticePropagator> &wall_props) {
  ...
  parallel_for(int ss=0; ss<ret._grid->lSites(); ss++){
    ...
    // v_to_wall = gamma5 * adj(wall_to_v) * gamma5;

    ret_site = 0.;
    for(int nu=0; nu<4; ++nu) {
      ret_site()()(nu) = trace(gamma5 * Gamma::gmu[nu] * wall_to_v * adj(wall_to_v));
    }

    // FIXME: how to deal with exponential??
    // ret_site = ret_site * (1 /  std::exp(- M_PION * t_min));  // FIXME: should position of wall depends on both u and v ?????
